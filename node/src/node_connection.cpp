    ...
    int payload_size)
{
    ...
    // Combine header and payload into a single buffer to avoid split TCP writes
    std::vector<uint8_t> packet(sizeof(header) + payload_size);
    ...
    if (!session.sendData(packet.data(), packet.size()))
    {
