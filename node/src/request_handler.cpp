...
RequestHandler::handle(const RequestResponseHeader& header, const uint8_t* payload, int payloadSize)
{
    ...
    if (g_cache.lookup(query, cached_data))
    {
        ...
        return makeResponse(
            query.oracleQueryId, cached_data.errorFlag, cached_data.payload.data(), cached_data.payload.size());
    }
    else    // miss cache
    {
        // Send full query to interface client and get full package {RequestRespondHeader + OracleReply + payload}
        ...
        memcpy(&reply, replyFullData.data() + sizeof(RequestResponseHeader), sizeof(OracleMachineReply));
        ...
        // Prepare payload
        std::vector<uint8_t> replyPayload(replyFullData.size() - sizeof(RequestResponseHeader) - sizeof(OracleMachineReply));
        memcpy(replyPayload.data(), replyFullData.data() + sizeof(RequestResponseHeader) + sizeof(OracleMachineReply), replyPayload.size());

        // Cache the payload
        // TODO: verify reply.oracleMachineErrorFlags
        g_cache.store(query, replyPayload.data(), replyPayload.size(), reply.oracleMachineErrorFlags);
        
        // Return success response with reply data
        return makeResponse(
            query.oracleQueryId, ORACLE_FLAG_REPLY_RECEIVED, replyPayload.data(), replyPayload.size());
    }
}
